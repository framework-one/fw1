	...
	private struct function processRouteMatch( string route, string target, string path ) {
		...
		if ( routeLen ) {
			...
			if ( route == '*' ) {
				...
				// only add the trailing slash if the last character is not a slash or $ (end of string)
				route &= '/';
			}
		} else {
			route = '/';
		...
		// add trailing match/back reference: if the last character is not a $ (end of string)
		if (right( route, 1 ) != '$')
			route &= '(.*)';
		target &= chr(92) & n;
