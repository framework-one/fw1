component {
    ...
    */
    ...
    /*
     * returns the loaded render utility component
     */
    ...
    public any function onRequest( string targetPath ) {
        ...
        if ( structKeyExists( request._fw1, 'renderData' ) ) {
            out = getRenderer().renderDataWithContentType( request._fw1.renderData );
        } else {
            buildViewQueue();
	...
	public void function setRenderer( any renderer ) {
		if ( isObject( renderer ) ) {
			getFw1App().renderer = renderer;
		} else {
			structDelete( getFw1App(), "renderer" );
		}
	}
